...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            ...
            // Get rectangles from corners
            for (int k = 0; k < n; k++) {
                if (k == i or k == j) continue;
                V v{a[i], a[k]};
                if (v.orthogonal({a[j], a[k]})) ve.push_back(a[k]);
            }

            for (int p1 = 0; p1 < ve.size(); p1++) {
                for (int p2 = p1 + 1; p2 < ve.size(); p2++) {
                    // Iterate over each compatible (perp.) pair.
                    vec<P> v{ve[p1], ve[p2]};
                    V v0{a[i], v[0]};
                    if (not v0.orthogonal({a[i], v[1]})) continue;

                    // one pass for each pair of perpendicular sides
                    for (int r = 0; r < 2; r++) {
                        // Find the period for the letter, by checking the directionality
                        for (int k = 0; k < n; k++) {
                            ftype c1 = (v[r] - a[i]).cross(a[k] - a[i]);
                            ftype c2 = (a[j] - v[!r]).cross(a[k] - v[!r]);
                            if (eq(c1, 0) or eq(c2, 0) or sign(c1) != sign(c2)) continue;
                            c1 = (v[!r] - a[i]).cross(a[k] - a[i]);
                            c2 = (a[j] - v[r]).cross(a[k] - v[r]);
                            if (eq(c1, 0) or eq(c2, 0) or sign(c1) == sign(c2)) continue;
                            cnt++;
                        }
                    }
