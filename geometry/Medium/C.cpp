...
void solve() {
    ...
    double up = N.z * A.x - A.z * N.x;
    double under = N.y * A.x - A.y * N.x;
    double coy = -up / under;
    double cox = -(A.z + A.y * coy) / A.x;
    double z = l / (2 * sqrt(sqr(coy) + sqr(cox) + 1));
    double x = cox * z;
    double y = coy * z;

    P b = {x, y, z}, c{-x, -y, -z}, t = A * b;
    if (lse(t.z * N.z, 0)) swap(b, c);
    P una{-A.x / 2, -A.y / 2, -A.z / 2};
    P B = una + b, C = una + c;
    la = A.dist(H), lb = B.dist(H), lc = C.dist(H);
//    cout << A << nl << B << nl << C << nl << nl;
    cout << la << ' ' << lb << ' ' << lc << nl;
}
