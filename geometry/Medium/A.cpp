...
//    }
    ...
    auto rCmp = [](const P& p0, const P& p1) -> bool {
        int a = p0.norm(), b = p1.norm();
        return a == b ? p0 < p1 : a < b;
    };
...
pii normal(const pii& p) {
    return reduce(-p.S, p.F);
}

using pd = pair<double, pii>;
bool eqf(double a, double b) {
    return abs(b - a) <= eps;
}
bool lsf(double a, double b) {
    return b - a > eps;
}
...
void solve() {
    ...
    vec<P> a(n);
    for (P& p: a) cin >> p;
    auto cmp = [](const pd& a, const pd& b) -> bool {
        return eqf(a.F, b.F) ? a.S < b.S : lsf(a.F, b.F);
    };

    int cnt = 0;
    for (int i = 0; i < n; i++) {
        map<pii, ordered_set<pd, decltype(cmp)>> m;
        for (int j = 0; j < n; j++) {
            if (i == j) continue;
            P v{a[j] - a[i]};
            m[v.normalize_p()].insert({v.abs(), {i, j}});
        }
        for (auto& [v, s]: m) {
            const auto& q = m.find(normal(v));
            if (q == m.end()) continue; // no normal
            for (const auto& [f, _]: s) {
                int li = (int)q->S.order_of_key({2 * l / f, {-INF, -INF}});
                int ri = (int)q->S.order_of_key({2 * r / f, {INF, INF}});
                cnt += ri - li;
            }
        }
    }
