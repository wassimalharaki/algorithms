 ...
 * C
 */
// ./algorithms/geometry/Hard/C.cpp
#include <bits/stdc++.h>
...
vec<P> convexHullMonotone(vec<P> a, bool include_collinear = false) {
...
int area(const vec<P>& pts) {
    int res = 0;
    for (int i = 0; i < pts.size(); i++) {
        P p = i ? pts[i - 1] : pts.back(), q = pts[i];
        res += (p.x - q.x) * (p.y + q.y);
    }
    return abs(res);
}
...
void solve() {
    ...
    for (P& p: a) cin >> p;
    setPrec();
    vec<P> pg = convexHullMonotone(a);
    int m = (int)pg.size();
    if (m <= k) {
        cout << c * area(pg) * 0.5l << nl;
        cout << m << nl;
        for (P& p: pg) cout << p << nl;
        return;
    }
    int mx = 0;
    vec<P> lt;
    ...
    while (s < k) {
        ...
        for (const P& p: ch) {
            vec<P> tmp(lt);
            tmp.push_back(p);
            const auto& tch = convexHullMonotone(tmp);
            int d = area(tch);
            if (d > cur) {
                cur = d;
                ad = p;
