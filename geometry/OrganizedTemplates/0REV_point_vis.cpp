...
// and 'p2q2' intersect.
bool doIntersect(point2d p1, point2d q1, point2d p2, point2d q2, bool igsame = true)
{
    if (igsame and (p1 == p2 or p1 == q2 or q1 == p2 or q1 == q2)) {
        return false;
    }
...
bool doIntersect(const PolySegment& s, const Poly& p) {
    ...
    point2d mid = 0.5 * (s.F + s.S);
    return not contains(mid, p);
}
...
// O(Elog(V))
vector<double> dijkstra(int src, const vector<vector<pair<double, int>>>& adj) {
    int n = adj.size();
...
void solve() {
    ...
    // we need to check whether a line segment between 2 vertices lies outside the polygon.
    // If so then there is no path.

    vec2d<pair<double, int>> adj(n + 1);
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (i == j) continue;
            PolySegment v{a.p[i], a.p[j]};
            if (not doIntersect(v, as)) adj[i].emplace_back(len(v), j);
        }
        const auto& v = PolyPDistanceVec(a.p[i], viss);
        if (not doIntersect(v, as)) adj[i].emplace_back(len(v), n);
    }
//    for (auto& r: adj) {
//        for (auto& p: r) {
//            cout << "(" << p.F << " - " << a.p[p.S] << ")";
